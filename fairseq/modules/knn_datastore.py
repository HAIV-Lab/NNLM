...
class KNN_Dstore(object):
    ...
    def get_keys_random(self, vals):
        # 内存不断增长
        if vals in self.prototype_keys.keys():
            select_keys = self.prototype_keys[vals]
        else:
            select_keys = self.keys[self.vals.cpu().numpy().reshape(-1)==vals][:10]
            select_keys = torch.from_numpy(select_keys).cuda().squeeze(-1).float()
            self.prototype_keys[vals] = select_keys
        return select_keys
    ...
    def retrieve(self, queries):
        ...
        # knn_keys = torch.from_numpy(self.keys[knns.cpu().numpy()]).to(queries.device).squeeze(-1).float()
        # knn_keys = knn_keys.view(bsz, seq_len, self.k, -1) 
