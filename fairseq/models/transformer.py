...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def build_model(cls, args, task):
        ...
        if args.only_train_knn_parameter:
            ...
            for name, param in decoder.named_parameters():
                ...
                if "knn_distances_to_lambda" in name and decoder.knn_lambda_type == "trainable":
                ...
                if decoder.train_compact_network is True and "compact" in name:
                    param.requires_grad = True

                if decoder.train_lambda_predict is True and "lambda_predict" in name:
                    param.requires_grad = True
    ...
    def forward_compact_network(self, features):
        self.decoder.forward_compact_network(features)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.num_layers = len(self.layers)
        self.train_compact_network = True
        self.compact_layer = self.build_compact(args, self.embed_dim)
        self.compact_network = self.build_FFN(self.embed_dim, len(dictionary))
        ...
        # TODO need a network to predict lambda 
        # input: nmt probs and compact probs
        # output: lambda
        self.train_lambda_predict = False
        self.lambda_predict_network = nn.Sequential(
            nn.Linear(len(dictionary) * 2 , args.k_lambda_net_hid_size),
            nn.ReLU(),
            nn.Linear(args.k_lambda_net_hid_size, 1),
            nn.Sigmoid()
        )

        self.compute_knn_frequency(args=args)
        ...
        if self.knn_lambda_type == "trainable" and self.knn_k_type == "trainable":
            ...
            if self.knn_lambda_type == 'trainable':
                ...
                self.knn_distances_to_lambda = nn.Sequential(
                    ...
                    nn.Linear(len(dictionary), args.knn_lambda_net_hid_size),
                    nn.Tanh(),
    ...
    def build_FFN(self, input_dim, output_dim):
        dropout = 0.
        compact_layer = nn.Sequential(
        ...
        # compact_layer = compact_layer.half()
        return compact_layer

    def build_compact(self, args, input_dim):
        compact_layer = nn.Sequential(
            nn.Linear(input_dim, input_dim // 2),
            nn.ReLU(),
        ...
        compact_layer = self.build_decoder_layer(self.args)
        return compact_layer
    
    def compute_knn_frequency(self, args):
        if not args.dstore_filename:
            raise ValueError('Cannot compute frequency without the data.')

        keys = np.memmap(self.args.dstore_filename + '/keys.npy', dtype=np.float16,
                              mode='r', shape=(self.args.dstore_size, self.embed_dim))

        values = np.memmap(self.args.dstore_filename + '/vals.npy', dtype=np.int, 
                                mode='r',shape=(self.args.dstore_size, 1))

        keys =  np.asarray(keys)
        values = np.asarray(values)

        self.frequency_values = [0 for i in range(len(self.dictionary))]
        self.total_keys = len(keys)
        for i in range(len(values)):
            self.frequency_values[values[i][0]] += 1
        self.frequency_values = np.array(self.frequency_values)
    
    def get_knn_frequency(self, targets):
        frequency = self.frequency_values[targets.cpu().numpy()]
        # frequency = 1.0 - frequency/self.total_keys
        return torch.from_numpy(frequency).cuda().float()

    def build_discriminate_network(self, input_dim):
        distcriminate_network = nn.Sequential(
            nn.Linear(input_dim, 2)
        )
        return distcriminate_network
    ...
    def forward(
        ...
        if self.train_compact_network:
            compact_hidden = self.compact_layer(x)
            compact_output = self.compact_network(x)
        else:
            compact_output = None
        ...
        if self.use_knn_datastore:
            ...
            # knn_keys = knn_search_result['knn_keys']
            ...
            if self.label_count_as_feature:
                ...
                network_inputs = compact_output.detach()
            ...
            if self.knn_lambda_type == "trainable" and self.knn_k_type == 'trainable':
                ...
                if self.knn_lambda_type == 'trainable':
                    ...
                    if self.train_lambda_predict is True:
                        knn_lambda = self.lambda_predict_network(torch.cat([x, compact_output], dim=-1))
                    else:
            ...
            if self.label_count_as_feature:
                return x, extra, knn_prob, knn_lambda, knn_dists, knn_index, label_counts, compact_output
                return x, extra, knn_prob, knn_lambda, knn_dists, knn_index, label_counts, compact_hidden, knn_keys, tgt_index, compact_output
            else:
                return x, extra, knn_prob, knn_lambda, knn_dists, knn_index, None, compact_output
                return x, extra, knn_prob, knn_lambda, knn_dists, knn_index, None, compact_hidden, knn_keys, tgt_index, compact_output
    ...
    def get_normalized_probs(
        ...
        if self.train_compact_network is True:
            compact_output = net_output[-1]
            compact_probs = utils.softmax(compact_output, dim=-1, onnx_trace=self.onnx_trace)
        else:
            compact_probs = None
        
        # TODO: 结合compact的分布与NMT的输出分布
        ...
        if self.use_knn_datastore:
            ...
            # logging.info(f"knn lambda {knn_lambda}")
            if self.train_compact_network is True:
                interpolation_probs = compact_probs
            else:
                interpolation_probs = knn_probs
            # print("### knn_probs shape:", knn_probs.shape)

            if self.knn_lambda_type == "fix":             
                probs = network_probs * (1 - knn_lambda) + interpolation_probs * knn_lambda
            else:
                probs = network_probs * (1 - knn_lambda) + interpolation_probs * knn_lambda

            if log_probs:
                return torch.log(probs), torch.log(compact_probs), network_probs
            else:
                return probs, compact_probs, network_probs

        if log_probs:
            return utils.log_softmax(logits, dim=-1, onnx_trace=self.onnx_trace), None, None
        else:
            return utils.softmax(logits, dim=-1, onnx_trace=self.onnx_trace), None, None
